...
## DEC-144 — Design-Time DbContext Factory & Repo-Pinned dotnet-ef Tool

- **Date:** 2026-07-17
- **Status:** Accepted
- **Context:** The runtime connection-string guard added to `Program.cs` (fail fast when `DefaultConnection` is missing/empty) broke design-time EF operations: `dotnet ef migrations script --idempotent` built the API host, the guard threw, and — with no design-time factory — EF then failed with *"Unable to resolve service for type DbContextOptions<ApplicationDbContext>"*. The migration workflow also requested `dotnet-ef 8.0.11` but the runner executed a different global version (10.0.10), and the first `--no-build` attempt looked for Debug binaries while the workflow built Release.
- **Decision:**
    1. **`DesignTimeDbContextFactory : IDesignTimeDbContextFactory<ApplicationDbContext>`** in `AlplaPortal.Infrastructure/Data/`. It builds `DbContextOptions` with `UseSqlServer` + `MigrationsAssembly("AlplaPortal.Infrastructure")`, resolves the connection string as env `ConnectionStrings__DefaultConnection` → optional local `appsettings(.Development).json` → a **non-operational design-time placeholder** (a `.invalid` server, never LocalDB), and never resolves app services, seeds, or opens a connection.
    2. **`--project` and `--startup-project` both target `AlplaPortal.Infrastructure`** for design-time SQL generation. Infrastructure is self-contained (DbContext + migrations + SqlServer provider + EFCore.Design + factory) and has **no host**, so `Program.cs` is never executed — the runtime guard stays intact but never fires at design time.
    3. **Repo-pinned local tool**: `.config/dotnet-tools.json` fixes `dotnet-ef 8.0.11`. Scripts/workflows run `dotnet tool restore`, then assert `dotnet ef --version` contains `8.0.11` and fail otherwise. No global tool is installed/updated/uninstalled on the runner.
    4. **Deterministic generation**: a single `dotnet ef migrations script --idempotent --configuration Release --no-build` reusing the workflow's Release build (no predictably-invalid Debug `--no-build` retry). Generation failure aborts before any SQL is applied.
- **Alternatives considered:** LocalDB placeholder fallback (rejected: a connecting command could silently act on an unintended database); relaxing/removing the runtime guard (rejected: guard is required for runtime safety and must stay unchanged); controlled global-tool uninstall/reinstall (rejected in favor of a local manifest, which is deterministic and non-invasive to the runner).
- **Consequences:** `dotnet ef` generates the idempotent script offline, without a real connection string and without executing `Program.cs`; connecting commands still require a real connection string and fail clearly (never LocalDB) when it is absent. Applies to both `apply-migrations-test.yml` and `apply-migrations-prod.yml` (same strategy).
- **Related:** [DEC-139](#dec-139), DEPLOYMENT_CHECKLIST.md § EF Core Migration Checklist

---

## DEC-145 — Incremental (FROM/TO) Migration Scripting with Strict Prefix Validation

- **Date:** 2026-07-17
- **Status:** Accepted
- **Context:** The migration workflow generated a full idempotent script from the FIRST migration. That re-emits the *body* of already-applied historical migrations. After a historical migration (`AddDepartmentManagers` seed, `PhaseCRemoveLegacyAreaApprovalConfig` backup) that contains raw `migrationBuilder.Sql()` referencing `Departments.ResponsibleUserId` — a column dropped later by the same Phase C migration — SQL Server **fails to COMPILE** the inline column reference inside the guarded `IF NOT EXISTS(...) BEGIN ... END` block (error 207 *"Invalid column name 'ResponsibleUserId'"*) **before** the runtime guard can skip it. Raw `Sql()` is emitted inline, unlike EF's own `InsertData` which is wrapped in `EXEC(N'...')` (deferred compilation).
- **Decision:**
    1. **Strict prefix validation** before any backup/generation/application: the applied migrations must be an **exact, contiguous prefix** of the filesystem list (project's canonical order from `get-expected-migrations.ps1`). Positional comparison `expected[0..n-1] == applied[0..n-1]`. Blocks on: applied-not-in-filesystem, more-applied-than-expected, out-of-position, gap, interleaved pending, divergent order, duplicates — reporting index/expected/found. The history is **never** auto-corrected.
    2. **Incremental range generation**: `dotnet ef migrations script <FROM> <TO> --idempotent`. `FROM` = last applied (or `0` for an empty DB — the full history is expected and correct there, since the column is created then dropped in sequence); `TO` = last filesystem migration. This excludes historical bodies (and their dropped-column references).
    3. **Exact-set SQL validation**: extract the MigrationIds the script INSERTs into `__EFMigrationsHistory` (not a generic substring search) and require they equal the pending set exactly — none already-applied, none after `TO`, count == pending — and (incremental scenario only) assert no `ResponsibleUserId` and no `SET QUOTED_IDENTIFIER OFF`. Any mismatch aborts **before** `sqlcmd`.
    4. **No-pending / empty-DB** behavior preserved: no pending → early success (no backup/script/sqlcmd); empty DB → `FROM = 0` full script.
    5. **Post-failure guidance**: since the EF idempotent script uses multiple GO-separated batches with **no global transaction**, failures are not guaranteed atomic. On `sqlcmd` failure the script prints a READ-ONLY `__EFMigrationsHistory` snapshot and explicitly requires manual verification of history + objects **before** any restore or retry (never auto-restores). A standalone `scripts/db/check-pre-migration-state.ps1` runs the read-only confirmation.
