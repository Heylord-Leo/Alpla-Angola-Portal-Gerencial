...
- **Decision:**
    ...
    2. **Strict Innux Database Segregation & Approved SQL Instance:** Under no circumstances will any existing `Innux`, `Innuxtime`, or `INUTIME` attendance databases on `AOVIA1VMS011` be touched, modified, or reused. The local databases `[Portal-Gerencial]` and `[Portal-Gerencial-Test]` must co-exist on the default general-purpose SQL Server 2019 instance (`MSSQLSERVER`), which has been officially approved. Databases will have completely isolated resources, schemas, and permissions to guarantee operational safety.
