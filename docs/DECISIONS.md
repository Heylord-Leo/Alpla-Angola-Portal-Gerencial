...
- **Decision:**
    ...
    2. **Strict Innux Database Segregation:** Under no circumstances will any existing `Innux`, `Innuxtime`, or `INUTIME` attendance databases on `AOVIA1VMS011` be touched, modified, or reused. The `[Portal-Gerencial]` database must co-exist on a general-purpose SQL instance (`MSSQLSERVER` or `MSSQLSERVER01`) with completely isolated resources, schemas, and permissions to guarantee operational safety.
