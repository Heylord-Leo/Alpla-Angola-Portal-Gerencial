...
- **Queue unit is now the actionable `ApprovalBatch` (`approvalBatchId` + `approvalStage`), not the Request**: `GET /requests/pending-approvals` returns one row per actionable batch (new `ApprovalQueueItemDto`, built by `ApprovalQueueProjection`), replacing the one-row-per-request projection that picked `OrderBy(BatchNumber).FirstOrDefault()`. A request with two simultaneously actionable batches now yields two independent cards. Resolves the v2.215.0 known limitation: **REQ-21/07/2026-132** now shows two Area Approval cards — Lote #1 (AOA 150.217,07 · AFRICANA DISCOUNT, LDA.) and Lote #2 (AOA 618.732,10 · Mistoquímica - Indústria Química, Lda) — each opening its own exact lot.
- **Explicit card→drawer batch identity (parity)**: the clicked card carries its exact `approvalBatchId` and the drawer opens that precise batch instead of independently re-selecting one by status; supplier, value, item count, status and navigation all come from the same batch. A defensive invariant surfaces a diagnostic banner if card and drawer ever disagree, rather than silently opening a different lot.
...
- **KPI semantics changed to actions**: primary card is now **"Ações Pendentes"** with a secondary **"X ações em Y pedidos"**; section badges count actionable approval actions and the queue monetary total sums each actionable row exactly once.
