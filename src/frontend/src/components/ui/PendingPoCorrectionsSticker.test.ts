...
describe('PendingPoCorrectionsSticker — v2.242.0', () => {
  ...
  it('CTA deep-links to the cross-type personal work surface (Para Minha Ação)', () => {
    // Cross-type corrections (QUOTATION + PAYMENT) live in /requests "Para Minha Ação", not the
    // QUOTATION-only Buyer queue.
    expect(sticker).toMatch(/\/requests\?isAttention=true/);
