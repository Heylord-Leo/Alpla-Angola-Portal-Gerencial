...
import { Pencil, Save, Download, Trash2, RefreshCw, AlertTriangle, CheckCircle, Clock, ArrowLeft } from 'lucide-react';
import { api } from '../../lib/api';
import { Feedback, FeedbackType } from '../../components/ui/Feedback';
import {
  SupplierFichaCapabilities, readCapabilities, fieldEditability, isFichaDirty,
} from './supplierFichaContent';
import './SupplierFicha.css';

// Phase 3D / Layer C — the SINGLE, reusable Supplier Ficha content. Extracted verbatim from the former
// SupplierFichaDetail route component: same fields, APIs, validation, completeness, documents and save
// semantics. The route page and (future) Buyer Workspace drawer both host THIS component; there is no
// second Supplier form. Authorization comes only from the backend `capabilities` on GET /ficha — no role
// mapping in React. `hostMode` adjusts layout only; behavior is identical across hosts.
...
interface SupplierFichaData {
  ...
  // Phase 3D — backend-resolved capabilities (authoritative UI decision source).
  capabilities?: SupplierFichaCapabilities;
}
...
export interface SupplierFichaDetailContentProps {
  /** Passed explicitly by the host — the content NEVER reads useParams (so a drawer can host it). */
  supplierId: number;
  /** 'page' preserves the current Contracts route UX; 'drawer' only adapts layout (future Workspace host). */
  hostMode: 'page' | 'drawer';
  /** Back/close affordance. Page host navigates to the list; drawer host closes the drawer. */
  onClose?: () => void;
  /** Fired after a successful ficha save (in addition to the existing in-place refresh) — for host refresh. */
  onSaved?: () => void;
  /** Surfaces whether the form has unsaved edits (for the future drawer close-guard; no blocking here). */
  onDirtyChange?: (dirty: boolean) => void;
}

const SupplierFichaDetailContent: React.FC<SupplierFichaDetailContentProps> = ({
  supplierId, hostMode, onClose, onSaved, onDirtyChange,
}) => {
  ...
  // Backend is the authoritative source of what this user may do to THIS supplier.
  const caps = readCapabilities(ficha);
  const fe = fieldEditability(caps, editMode);

  const fetchFicha = async () => {
    if (!supplierId) return;
    setLoading(true);
    try {
      const data = await api.lookups.getSupplierFicha(supplierId);
      setFicha(data);
      const nextForm = {
        name: data.name || '',
        taxId: data.taxId || '',
  ...
  // Surface a dirty signal for the future drawer close-guard (no navigation blocking in this layer).
  useEffect(() => {
    onDirtyChange?.(editMode && isFichaDirty(formData, snapshot));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData, snapshot, editMode]);

  const fetchCompleteness = async () => {
    if (!supplierId) return;
    try {
      const data = await api.lookups.getSupplierCompleteness(supplierId);
  ...
  const fetchHistory = async () => {
    if (!supplierId) return;
    try {
      const data = await api.lookups.getSupplierHistory(supplierId);
      setHistory(data);
    } catch { /* silent */ }
  };
  ...
  if (!ficha) {
    return (
      <div className="ficha-detail-loading">
        ...
        <button className="ficha-btn ficha-btn--secondary" onClick={() => onClose?.()}>Voltar</button>
      </div>
    );
  }

  const docCompleteness = REQUIRED_DOC_TYPES.filter(t => ficha.uploadedDocTypes.includes(t)).length;
  // Governance / status controls are visible only when the backend grants the capability.
  const statusActions = caps.canChangeStatus ? getStatusActions() : [];

  return (
    <div className={`ficha-detail-container${hostMode === 'drawer' ? ' ficha-detail-container--drawer' : ''}`}>
      {/* Feedback */}
      <Feedback
        type={feedback.type}
      ...
      <div className="ficha-detail-header">
        <button className="ficha-back-btn" onClick={() => onClose?.()} aria-label="Voltar">
        ...
        <div className="ficha-detail-header-info">
          ...
          <div className="ficha-detail-meta">
            ...
            {/* Drawer host shows the NIF in the (compact) header so it reads as a proper supplier header
                without a second large header block; page host is unchanged. */}
            {hostMode === 'drawer' && ficha.taxId && <span>NIF: {ficha.taxId}</span>}
      ...
      {completeness && !['ACTIVE', 'SUSPENDED', 'BLOCKED'].includes(ficha.registrationStatus) && (
        <div className="ficha-completeness-panel">
          ...
          {/* Submit for Approval Button — only when the backend grants the capability. */}
          {caps.canSubmitForApproval && ['DRAFT', 'PENDING_COMPLETION', 'ADJUSTMENT_REQUESTED'].includes(ficha.registrationStatus) && (
            <button
      ...
      <div className="ficha-detail-grid">
        ...
        <div className="ficha-section">
          ...
          <div className="ficha-field-grid">
            <FieldRow label="Denominação Social" field="name" value={formData.name} editMode={fe.identity} onChange={handleInputChange} required />
            <FieldRow label="NIF / Contribuinte N.º" field="taxId" value={formData.taxId} editMode={fe.taxLegal} onChange={handleInputChange} />
            <FieldRow label="Código Primavera" field="primaveraCode" value={formData.primaveraCode} editMode={fe.identity} onChange={handleInputChange} />
            <FieldRow label="Morada" field="address" value={formData.address} editMode={fe.address} onChange={handleInputChange} />
          </div>
        </div>
        ...
        <div className="ficha-section">
          ...
          <div className="ficha-field-grid">
            <FieldRow label="Nome" field="contactName1" value={formData.contactName1} editMode={fe.contacts} onChange={handleInputChange} />
            <FieldRow label="Cargo" field="contactRole1" value={formData.contactRole1} editMode={fe.contacts} onChange={handleInputChange} />
            <FieldRow label="Telemóvel" field="contactPhone1" value={formData.contactPhone1} editMode={fe.contacts} onChange={handleInputChange} />
            <FieldRow label="E-mail" field="contactEmail1" value={formData.contactEmail1} editMode={fe.contacts} onChange={handleInputChange} type="email" />
          </div>
        </div>
        ...
        <div className="ficha-section">
          ...
          <div className="ficha-field-grid">
            <FieldRow label="Nome" field="contactName2" value={formData.contactName2} editMode={fe.contacts} onChange={handleInputChange} />
            <FieldRow label="Cargo" field="contactRole2" value={formData.contactRole2} editMode={fe.contacts} onChange={handleInputChange} />
            <FieldRow label="Telemóvel" field="contactPhone2" value={formData.contactPhone2} editMode={fe.contacts} onChange={handleInputChange} />
            <FieldRow label="E-mail" field="contactEmail2" value={formData.contactEmail2} editMode={fe.contacts} onChange={handleInputChange} type="email" />
          </div>
        </div>
        ...
        <div className="ficha-section">
          ...
          <div className="ficha-field-grid">
            <FieldRow label="Conta N.º" field="bankAccountNumber" value={formData.bankAccountNumber} editMode={fe.banking} onChange={handleInputChange} />
            <FieldRow label="IBAN" field="bankIban" value={formData.bankIban} editMode={fe.banking} onChange={handleInputChange} />
            <FieldRow label="SWIFT" field="bankSwift" value={formData.bankSwift} editMode={fe.banking} onChange={handleInputChange} />
          </div>
        </div>
        ...
        <div className="ficha-section">
          ...
          <div className="ficha-field-grid">
            <FieldRow label="Condições de Pagamento" field="paymentTerms" value={formData.paymentTerms} editMode={fe.commercial} onChange={handleInputChange} />
            <FieldRow label="Forma de Pagamento" field="paymentMethod" value={formData.paymentMethod} editMode={fe.commercial} onChange={handleInputChange} />
          </div>
        </div>
                  ...
                  {existingDoc ? (
                    ...
                    <div className="ficha-doc-card-body ficha-doc-card-body--empty">
                      {isUploading ? (
                        ...
                        <span className="ficha-doc-status ficha-doc-status--missing">Documento em falta</span>
                      )}
                    </div>
                  )}
...
export default SupplierFichaDetailContent;
