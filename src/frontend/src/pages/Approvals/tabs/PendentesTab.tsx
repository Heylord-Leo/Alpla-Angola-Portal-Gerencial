...
import { AlertCircle, AlertTriangle, Building2, User, Landmark, ShieldCheck, Inbox, ChevronRight, FileText as FileContract, Info, Clock } from 'lucide-react';
...
// v2.244.0 Approval Center V2 — PENDENTES tab.
// Re-homes the full pending-approval workspace (KPIs, triage, Area/Final queues, contracts, supplier
// fichas, decision drawer, keyboard nav, resizable drawer) under the PENDENTES tab, unchanged in
// behavior. The queue is presented in a denser operational layout with bounded internal scrolling so
// every actionable row stays reachable without a multi-page card wall. No business logic, scoping,
// projection, or mutation endpoint is touched — this is presentation-only.

// --- Types ---

type ApprovalStage = 'AREA' | 'FINAL';

// Aging derived purely from the existing createdAtUtc on the pending projection (no new backend).
...
function oldestAge(items: ApprovalQueueItemDto[]): string | null {
    let min = Number.POSITIVE_INFINITY;
    for (const it of items) {
        const t = new Date(it.createdAtUtc).getTime();
        if (!Number.isNaN(t) && t < min) min = t;
    }
    return Number.isFinite(min) ? formatAge(new Date(min).toISOString()) : null;
}
...
export function PendentesTab() {
    ...
    const areaTotal = data?.areaApprovals?.length || 0;
    const finalTotal = data?.finalApprovals?.length || 0;
    const oldest = oldestAge([...(data?.areaApprovals || []), ...(data?.finalApprovals || [])]);
    ...
    return (
        <>
            ...
            {/* Compact supporting counts — stage split + oldest, all derived from the current payload */}
            {(isAreaApprover || isFinalApprover) && totalPending > 0 && (
                <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'center', fontSize: '0.75rem', fontWeight: 700, color: 'var(--color-text-muted)', marginTop: '-12px' }}>
                    {isAreaApprover && <span>Área: <strong style={{ color: 'var(--color-text-main)' }}>{areaTotal}</strong></span>}
                    {isFinalApprover && <span>Final: <strong style={{ color: 'var(--color-text-main)' }}>{finalTotal}</strong></span>}
                    {oldest && (
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                            <Clock size={12} /> Mais antigo: <strong style={{ color: 'var(--color-text-main)' }}>{oldest}</strong>
                        </span>
                    )}
                </div>
            )}
            ...
            {/* QUEUE SECTIONS (Master) — denser, with bounded internal scroll per stage */}
                        ...
                        <div style={{
                            ...
                            <section style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderLeft: '4px solid #7c3aed', paddingLeft: '16px' }}>
                                    <span style={{ color: '#7c3aed' }}><FileContract size={18} /></span>
                                    <h2 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--color-text-main)', letterSpacing: '-0.02em' }}>
                                        Contratos Pendentes
                                    </h2>
                                    <Tooltip
                                        variant="light"
                    ...
                    <div style={{
                        ...
                        <section style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderLeft: '4px solid #d97706', paddingLeft: '16px' }}>
                                <span style={{ color: '#d97706' }}><FileContract size={18} /></span>
                                <h2 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--color-text-main)', letterSpacing: '-0.02em' }}>
...
// Beyond this many rows the queue scrolls INTERNALLY instead of extending the page — every row stays
// reachable via the bounded scroll area, never a "first N forever" truncation.
const QUEUE_SCROLL_MAX_HEIGHT = 560;
        ...
        return (
            <section style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderLeft: '4px solid var(--color-border)', paddingLeft: '16px' }}>
                    ...
                    <h2 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--color-text-muted)', letterSpacing: '-0.02em' }}>{title}</h2>
                    {tooltip && (
                ...
                <div style={{ padding: '32px 20px', textAlign: 'center', color: 'var(--color-text-muted)', border: '2px dashed var(--color-border)', backgroundColor: 'var(--color-bg-surface)', borderRadius: 'var(--radius-lg, 8px)' }}>
    ...
    return (
        <section style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderLeft: '4px solid var(--color-primary)', paddingLeft: '16px' }}>
                ...
                <h2 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--color-text-main)', letterSpacing: '-0.02em' }}>{title}</h2>
                {tooltip && (
                    <Tooltip variant="light" side="bottom" align="start" content={
                        <span style={{ fontSize: '0.82rem', lineHeight: 1.6 }}>{tooltip}</span>
                    }>
            ...
            {/* Card list — bounded internal scroll keeps every row reachable without a page-long wall */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: QUEUE_SCROLL_MAX_HEIGHT, overflowY: 'auto', paddingRight: 4 }}>
                {requests.map((req, i) => {
                    ...
                    const age = formatAge(req.createdAtUtc);
                    ...
                    return (
                        <motion.div
                            ...
                            whileHover={{ y: -1, boxShadow: '0 4px 12px rgba(0,0,0,0.08)' } as any}
                        >
                            {/* Left: request number + date + type */}
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                    {isSelected && <ChevronRight size={14} style={{ color: 'var(--color-primary)', flexShrink: 0 }} />}
                                    <span style={{ fontWeight: 800, fontSize: '0.88rem', color: 'var(--color-primary)' }}>{req.requestNumber}</span>
                                    {req.lotNumber != null && (
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3, flexWrap: 'wrap' }}>
                                    <span style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)', fontWeight: 600 }}>{formatDate(req.createdAtUtc)}</span>
                                    {age && (
                                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: '0.66rem', fontWeight: 700, color: 'var(--color-text-muted)' }}>
                                            <Clock size={10} /> {age}
                                        </span>
                                    )}
                                    {/* Type badge */}
                                    <span style={{ fontSize: '0.62rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', padding: '1px 7px', borderRadius: 4, border: '1px solid var(--color-border)', color: 'var(--color-text-muted)' }}>
                                        {req.requestTypeCode}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div style={{ minWidth: 0 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 700, fontSize: '0.85rem', color: 'var(--color-text-main)' }}>
                                ...
                                <div style={{ fontSize: '0.73rem', color: 'var(--color-text-muted)', fontWeight: 600, textTransform: 'uppercase', marginTop: 2 }}>
                                    {req.departmentName || '---'}
                                </div>
                                {req.supplierDisplay && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 3 }}>
                                        <Building2 size={11} style={{ color: 'var(--color-text-muted)', flexShrink: 0 }} />
                                        <span style={{ fontSize: '0.73rem', color: 'var(--color-text-main)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{req.supplierDisplay}</span>
                                    </div>
                                )}
                                {showCostCenter && req.costCenterCode && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 4 }}>
                                        <Landmark size={11} style={{ color: 'var(--color-primary)', flexShrink: 0 }} />
                                        <span style={{ fontWeight: 700, fontSize: '0.73rem', color: 'var(--color-text-main)' }}>{req.costCenterCode}</span>
                                    </div>
                                )}
                            </div>
                                ...
                                <div>
                                    ...
                                    {cardAmount.state === 'undefined' ? (
                                        ...
                                        <Tooltip variant="dark" content={<span style={{ fontWeight: 700, fontSize: '0.75rem' }}>O total aprovado do lote não coincide com a soma dos itens. Verifique antes de decidir.</span>}>
                                            <div style={{ fontWeight: 800, fontSize: '0.98rem', color: '#b45309', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 4, cursor: 'help' }}>
                                                <AlertTriangle size={13} /> {cardAmount.display}
                                            </div>
                                        </Tooltip>
                                    ) : (
                                        <div style={{ fontWeight: 800, fontSize: '0.98rem', color: 'var(--color-text-main)', whiteSpace: 'nowrap' }}>
                                            {cardAmount.display}
                                        </div>
                                    )}
                                </div>
