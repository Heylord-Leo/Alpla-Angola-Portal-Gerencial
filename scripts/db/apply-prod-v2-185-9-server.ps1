<#
.SYNOPSIS
    PRODUCTION Migration Application Script - v2.185.9
.DESCRIPTION
    ...
    This script was validated on TEST (Portal-Gerencial-Test) on 2026-06-04.
    The exact same migration logic is applied here targeting Portal-Gerencial.
.NOTES
    Database:   Portal-Gerencial
    Server:     AOVIA1VMS011 (localhost)
    App Pools:  AlplaPortal-Prod-Api-Pool, AlplaPortal-Prod-Web-Pool
#>
...
Write-Host " PROD Migration Application - v2.185.9 " -ForegroundColor Red
Write-Host "========================================" -ForegroundColor Red
Write-Host ""
...
# ---- CONFIRMATION GATE ----
$confirm = Read-Host "Are you sure you want to apply migrations to PRODUCTION [$database]? Type 'YES-PROD' to continue"
...
Write-Host "STEP 1/4 - Creating PRODUCTION database backup..." -ForegroundColor Yellow
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$backupDir = "C:\Apps\AlplaPortal\Prod\backups\db"
...
Write-Host "STEP 2/4 - Applying pending migrations to PRODUCTION..." -ForegroundColor Yellow

$tempSqlFile = Join-Path $env:TEMP "apply-prod-migrations-v2-185-9.sql"

@'
-- PRODUCTION - Portal-Gerencial - v2.185.9
...
if ($totalCount -eq "52") {
    Write-Host "OK - Total migration count is 52. PRODUCTION schema is aligned." -ForegroundColor Green
} else {
...
Write-Host " PROD MIGRATION APPLICATION COMPLETE    " -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Next steps:" -ForegroundColor White
Write-Host "  1. Re-run the v2.185.9 PRODUCTION deployment from GitHub Actions" -ForegroundColor White
Write-Host "  2. Verify the pending migration check passes" -ForegroundColor White
Write-Host "  3. Confirm the production health check returns 200" -ForegroundColor White
Write-Host "  4. Validate login at https://portalgerencial.alpla.net" -ForegroundColor White
Write-Host ""
